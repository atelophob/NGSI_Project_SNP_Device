...
THIS HAS BEEN MODIFIED FOR THE 45DEGREE COIL SETUP
This code runs just a few paths or many paths

THIS CODE IS STILL NOT COMPLETE--THE PATH BOUNDARY COORDINATES STILL NEED TO BE UPDATED #LINES 134-135
THE SAME APPLIES FOR THE PATH REFERENCE STARTING POINTS #LINES 157-160


How to use:
1. Use command/control f to search up the word many paths to find the part where you must comment out the many path section to do the few path calculation
    (lines 176)
2. Simply change the CRa value in its list and the problem number in its list (this code cannot currently do multiple CRa/problems)
3. Create a log file on your device that matches the name of the file on here
4. Change the name of your filepaths depending on your file name as well as the log location
    (2-4 are all around line 87 or so)
The code does not automatically save your figures.
Using spyder, you can easily save your plots from the plots tab.


NOTES:
    use plt.pause(10) when doing just 3 paths, in which the angstp value needs to be 0 for that, otherwise follow the comment there
    use plotflag = 1 for just 3 paths and 0 for doing more
    If running the plotflag = 1 version, turn statplotflag off and also use plt.pause(10) not plt.pause(0) 
WriteIV does not work, but to make a version of a video, simply at plt.show within the plotting if blocks at the end and then save all the images produced to compile for a gif/vid
Writedata has not been tested
"""
import numpy as np
...
filenameid = 'SNP Development 45 deg Tri' # name of .mn work file. The data files are named in a pattern as (.mn )-p(problem no.)_3DTable.txt.
loglocation = r'/Users/kean/Downloads' # file location
# loglocation = r'C:\Users\klx\OneDrive - Oak Ridge National Laboratory'

log_filepath = os.path.join(loglocation, f"{filenameid}_log.txt")

CRa = [.4] # current ratio (center coil /side coil)
pro = [2] # problem number
...
with open(log_filepath, 'a') as logid:
    ...
    #coil 1 - bottom left
    c1x = np.array([-4.021, -4.021])
    c1z = np.array([0.264, 1.534])
    # coil 2 - up left
    c2x = np.array([-3.039, -3.919])
    c2z = np.array([2.647, 1.767])
    # coil3 - bottom right
    c3x = np.array([-19.519, -19.519])
    c3z = np.array([0.264, 1.534])
    # coil 4 - center right
    c4x = np.array([-19.476, -14.887])
    c4z = np.array([1.576, 12.654])
    # coil 5 - up right
    c5x = np.array([-13.998, -14.878])
    c5z = np.array([13.607, 12.727])
    ...
    divang_Array = np.array([44.]) # scattering angle range to cover (horizontal), in deg
    bsz = 0.2 # beam size
    angnum = 3 # define path number to calculate or step size in deg
    angstp = 0.4 # set as 0 if define path number, else 0.2 or 0.4
    
    #few paths
    positionx_Array = np.array([0.]) # offset of center path starting point from the reference center path
    positionang_Array = np.array([0.]) # offset of center path orientation from the reference center path, in deg
    
    #many paths
    # positionx_Array = np.array([-.4,-.2,0,.2,.4]) # offset of center path starting point from the reference center path
    # positionang_Array = np.array([-3, -1, 2]) # offset of center path orientation from the reference center path, in deg
    ...
    ode23flag = 1 # ode method, 1-implicit solution, 0-explict solutio
    ...
    for icr in range(0, len(CRa)):
        ...
        for iBamp in range(0, len(Bamp)):
            ...
            for ilamda in range(0, len(lamdaneutron)):
                ...
                #writeiv code block does not work
                if writeiv == 1:
                        ...
                        for idivang in range(0, len(divang_Array)): # calculate for every divergent path
                            ...
                            for ix in range(0, angnum):
                                ...
                                if lang >= bd1ang and lang <= bd2ang: # for every unshadowed path
                                    ...
                                    fl = np.gradient(B_A_Adia,lstp, axis = 0) 
